...
public class DriveMotionPlanner implements CSVWritable {
    ...
    private boolean useDefaultVelocity = true;
    ...
    public void setTrajectory(final TrajectoryIterator<TimedState<Pose2dWithCurvature>> trajectory) {
        ...
        defaultVelocity = trajectory.trajectory().getDefaultVelocity();
    ...
    protected Translation2d updatePurePursuit(Pose2d current_state) {
        ...
        if (actual_lookahead_distance < Constants.kPathMinLookaheadDistance) {
            lookahead_state = new TimedState<>(new Pose2dWithCurvature(lookahead_state.state().getPose().transformBy(
                Pose2d.fromTranslation(
                        new Translation2d((mIsReversed ? -1.0 : 1.0) * (Constants.kPathMinLookaheadDistance - actual_lookahead_distance), 0.0)
        ...
        if(normalizedSpeed > defaultVelocity || mSetpoint.t() > (currentTrajectoryLength / 2.0)){
            useDefaultVelocity = false;
        }
